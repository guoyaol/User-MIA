...
df_shadow_mem = pd.read_csv(
    "/root/Attack_prid2011/datasets/prid_2011_plan2/shadow_split_record/shadow_mem.csv", index_col=0)
df_shadow_nonmem = pd.read_csv(
    "/root/Attack_prid2011/datasets/prid_2011_plan2/shadow_split_record/shadow_nonmem_new.csv", index_col=0)

index_shadow_mem = df_shadow_mem.to_numpy()
index_shadow_nonmem = df_shadow_nonmem.to_numpy()

mem_features = np.zeros((10,150,5))
nonmem_features = np.zeros((10,150,5))

with torch.no_grad():
    for count_shadow in range(10):
        print(count_shadow)

        model.load_state_dict(torch.load(
            "./saved_models_aug/"+str(count_shadow)+'.pkl'))
        print("model loaded")
        model.eval()
        ...
        for count_person in range(150):
            ...
            ds_mem = prid2011_single_noaug(
                '/root/Attack_prid2011/datasets/prid_2011_plan2/train_split_equal/'+str_current_person, is_train=False)
        ...
        for count_person in range(150):
            ...
            str_current_person = "person_"+str(current_person).zfill(4)
            ds_nonmem = prid2011_single_noaug(
...
attack_input = attack_input.reshape(2*10*150,5)

attack_labels = np.hstack((np.ones(10*150),np.zeros(10*150)))


df_input = pd.DataFrame(attack_input)
df_labels = pd.DataFrame(attack_labels)
df_input.to_csv('./attack_set/attack_input_train.csv')
df_labels.to_csv('./attack_set/attack_labels_train.csv')