...
def embd_list(model, id_list):
    ...
    with torch.no_grad():
        ...
        for current_person in id_list:
            str_person = "person_"+str(current_person).zfill(4)
            ds_mem = prid2011_single_noaug(
                './datasets/prid_2011_plan2/semi_split/'+str_person, is_train=False)
...
df_victim_mem = pd.read_csv(
    "./datasets/prid_2011_plan2/shadow_split_record/victim_mem.csv", index_col=0)
...
for each_mem in list_victim_mem:
    str_mem = "person_"+str(each_mem).zfill(4)
    name_list = os.listdir("./datasets/prid_2011_plan2/train_split/"+str_mem)   
...
for each_pair in pair_list:
    if each_pair[1]>=123 and each_pair[1] <= 445:
        tier_1.append(each_pair)
    elif each_pair[1]>=102:
...
with torch.no_grad():
    ...
    model.load_state_dict(torch.load("./saved_models_aug/"+'victim.pkl'))
    model.eval()
